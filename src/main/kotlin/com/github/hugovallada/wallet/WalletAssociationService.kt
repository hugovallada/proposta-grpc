    ...
    fun associate(request: AssociateWalletGrpcRequest): String {
        ...
        return try{
            val associate = creditCardClient.associate(
                creditCard.number,
                AssociateWalletClientRequest(email = wallet.email, carteira = wallet.name)
            )

            val wal = walletRepository.save(wallet)
            creditCard.associateWallet(wal)
            creditCardRepository.update(creditCard)

            "Credit card ${creditCard.number} associated with wallet ${wallet.name.toUpperCase()}"
        }catch (exception: HttpClientResponseException){
            "Credit card ${creditCard.number} not associated with wallet ${wallet.name.toUpperCase()}"
        }
    }
